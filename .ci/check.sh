...
git diff origin2/master urls.tsv | grep -v '^+++' | grep '^+' | awk -F'\t' '{print "queryHash=$(curl -L --silent "$5" | sha256sum - | sed \"s/  .*//g\" ); userHash="$7"; if [ \"$queryHash\" == \"$userHash\" ]; then echo \"Valid hash\"; else exit 1; fi;"}' | bash
