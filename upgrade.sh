...
cat urls.tsv | awk -F'\t' '(NR>1){a=$1"_"$2"_"$3"_"$4".tar.gz"; print "mkdir -p "$1"; mv "$6" "$1"/"a}' >> mkdir.txt
cat mkdir.txt | bash
...
echo 'set -x' > mv.txt
cat urls.tsv | awk -F'\t' '(NR>1){a=$1"_"$2"_"$3"_"$4".tar.gz"; print "echo "$6" >> "$1"/SHA256SUMS"}' >> mv.txt
cat mv.txt | bash
